...
public class NtpOverSocks5 : IDisposable
{
    ...
    private static byte[] BuildNtpClientRequestPacket()
    {
        ...
        ntpReq[0] = 0x23;
        return ntpReq; // LI=0,VN=4,Mode=3
    }
    ...
    public async Task<NtpViaProxyResult> GetNtpTimeAsync(string targetNtpServerHost, TimeSpan operationTimeout)
    {
        ...
        try
        {
            ...
            try
            {
                if (!IPAddress.TryParse(targetNtpServerHost, out targetNtpIp!))
                {
                    ...
                    targetNtpIp = hostEntry.AddressList.FirstOrDefault(ip => ip.AddressFamily == AddressFamily.InterNetwork) ?? // Prefer IPv4
            ...
            {
                AddressType = targetNtpIp.AddressFamily == AddressFamily.InterNetwork ? Socks5AddressData.AddrTypeIPv4 : Socks5AddressData.AddrTypeIPv6,
                Host = targetNtpIp.ToString(),
        ...
        catch (Exception ex)
        {
            ...
            Logging.SaveLog(_tag + ": GetNtpTimeAsync error", ex);
        }
        finally
        {
            Dispose();
    ...
    private async Task<bool> EstablishUdpAssociationAsync(CancellationToken cancellationToken)
    {
        ...
        {
            // UDP ASSOCIATE request failed by REP code
            return false;
        }
