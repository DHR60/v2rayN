...
public class SpeedtestService(Config config, Func<SpeedTestResult, Task> updateFunc)
{
    ...
    private async Task<List<ServerTestItem>> GetClearItem(ESpeedActionType actionType, List<ProfileItem> selecteds)
    {
        ...
        foreach (var it in selecteds)
        {
            ...
            lstSelected.Add(new ServerTestItem()
            {
                ...
                CoreType = AppManager.Instance.GetCoreType(it, it.ConfigType),
    ...
    {
        // _config.SpeedTestItem.UdpTestTarget is like
        // ntp:time.google.com
        // dns:[::1]
        // stun:stun.l.google.com:19302
        // ...
        var parts = _config?.SpeedTestItem.UdpTestTarget.Split(':', 2);
        var udpTestType = parts?.Length > 0 ? parts[0].ToLower() : "ntp";
        var udpTestUrl = parts?.Length > 1 ? parts[1] : "time.google.com";

        UdpService udpService = udpTestType.ToLowerInvariant() switch
        {
            "ntp" => new NtpService(),
            "dns" => new DnsService(),
