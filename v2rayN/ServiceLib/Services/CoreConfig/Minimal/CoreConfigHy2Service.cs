...
public class CoreConfigHy2Service(Config config) : CoreConfigServiceMinimalBase(config)
{
    protected override async Task<RetResult> GeneratePassthroughConfig(ProfileItem node, int port)
    {
        ...
        try
        {
            ...
            configJsonNode["server"] = address + ":" + outboundPort;
            ...
            if (node.StreamSecurity == Global.StreamSecurity)
            {
                ...
                {
                    ["sni"] = node.Sni,
                    ["insecure"] = node.AllowInsecure.Equals("true", StringComparison.CurrentCultureIgnoreCase)
                };
                var certs = CertPemManager.ParsePemChain(node.Cert);
                if (certs.Count > 0)
                {
                    var pinSHA256 = CertPemManager.GetCertSHA256Thumbprint(certs[0], true);
                    if (pinSHA256.IsNotEmpty())
                    {
                        configJsonNode["tls"]["pinSHA256"] = pinSHA256;
                        configJsonNode["tls"]["insecure"] = false;
                    }
                }
            }
