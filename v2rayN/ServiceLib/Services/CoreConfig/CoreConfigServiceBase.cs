...
public abstract class CoreConfigServiceMinimalBase(Config config)
{
    ...
    {
        return GeneratePassthroughConfig(node, AppManager.Instance.GetLocalPort(EInboundProtocol.split));
    }
...
public abstract class CoreConfigServiceBase(Config config) : CoreConfigServiceMinimalBase(config)
{
    ...
    public abstract Task<RetResult> GenerateClientMultipleLoadConfig(ProfileItem parentNode);
    public abstract Task<RetResult> GenerateClientChainConfig(ProfileItem parentNode);
