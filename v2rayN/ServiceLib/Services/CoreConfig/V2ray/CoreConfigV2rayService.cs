...
public partial class CoreConfigV2rayService(Config config) : CoreConfigServiceBase(config)
{
    ...
    protected override async Task<RetResult> GeneratePassthroughConfig(ProfileItem node, int port)
    {
        ...
        try
        {
            ...
            // set direct detour for avoid loop, if tun enabled
            if (_config.TunModeItem.EnableTun && _config.TunModeItem.EnableExDirectPorts)
            {
                var directOutbound = new Outbounds4Ray
                {
                    tag = EInboundProtocol.dirdet.ToString(),
                    protocol = Global.ProtocolTypes[EConfigType.SOCKS],
                    settings = new Outboundsettings4Ray
                    {
                        servers = new List<ServersItem4Ray>
                        {
                            new()
                            {
                                address = Global.Loopback,
                                port = AppManager.Instance.GetLocalPort(EInboundProtocol.dirdet),
                            },
                        },
                    },
                };

                // Set all outbound detours to direct, excluding those that already have one.
                foreach (var outbound in v2rayConfig.outbounds)
                {
                    if (!(outbound.streamSettings?.sockopt?.dialerProxy?.IsNullOrEmpty() ?? true))
                    {
                        continue;
                    }
                    outbound.streamSettings ??= new StreamSettings4Ray();
                    outbound.streamSettings.sockopt ??= new Sockopt4Ray();
                    outbound.streamSettings.sockopt.dialerProxy = directOutbound.tag;
                }

                v2rayConfig.outbounds.Add(directOutbound);
            }

            ret.Msg = string.Format(ResUI.SuccessfulConfiguration, "");
            ret.Success = true;

            var config = JsonUtils.ParseJson(JsonUtils.Serialize(v2rayConfig)).AsObject();
