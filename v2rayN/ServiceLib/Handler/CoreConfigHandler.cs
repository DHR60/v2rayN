...
public static class CoreConfigHandler
{
    ...
    public static async Task<RetResult> GenerateClientConfig(ProfileItem node, string? fileName)
    {
        ...
        else if (AppManager.Instance.GetCoreType(node, node.ConfigType) == ECoreType.sing_box || (Global.SingboxSupportConfigType.Contains(node.ConfigType) && !Global.XraySupportConfigType.Contains(node.ConfigType)))
        {
    ...
    public static async Task<RetResult> GeneratePureEndpointConfig(ProfileItem node, string? fileName)
    {
        ...
        var coreType = AppManager.Instance.GetSplitCoreType(node, node.ConfigType);

        var result = coreType switch
        {
