...
public class CoreManager
{
    ...
    public async Task LoadCore(ProfileItem? node)
    {
        ...
        var context = new CoreLaunchContext(node, _config).AdjustForConfigType();
    ...
    {
        var coreType = selecteds.Any(t => t.CoreType == ECoreType.sing_box || Global.SingboxOnlyConfigType.Contains(t.ConfigType)) ? ECoreType.sing_box : ECoreType.Xray;
    ...
    private async Task<bool> CoreStart(CoreLaunchContext context)
    {
        ...
        AppManager.Instance.RunningCoreType = context.GetInRouteCoreType();
        return true;
    }
    ...
    private async Task<bool> CoreStartPreService(CoreLaunchContext context)
    {
        ...
        // itemSocksLaunch..AdjustForConfigType()

        // why not adjust here?
        // because the AdjustForConfigType() will change the CoreType to other type instead of context.GetInRouteCoreType()
